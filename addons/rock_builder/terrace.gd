...
func proposal() -> Dictionary:
 ...
 for edge in 4:
  ...
  for i in count:
   var id := "%d_%d"%[edge,i]; ids.append(id)
   if not existing.has(id) and prior.ids.has(id): continue
   if existing.has(id) and is_authored(existing[id]):
    records.append(existing[id]); continue
 ...
 return {"platform":{"footprint":footprint,"elevation":elevation},"rocks":{"records":records,"ids":ids}}
