@tool
extends Node3D
@export_enum("Perimetro","Strada","Tipo di quartiere","Area non edificabile","Corte di proprietà") var kind := 0
...
func _process(_dt: float) -> void:
	...
	var signature := str(points,road_width,kind,surfaced,show_zones)
	...
	if (show_zones or not Engine.is_editor_hint()) and (kind==4 and (not surfaced or Engine.is_editor_hint()) or (kind in [2,3] and Engine.is_editor_hint())) and points.size()>=3:
		...
		return
	if kind!=1 or points.size()<2 or surfaced: return
