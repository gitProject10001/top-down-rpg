# Archivio della consegna — NPC conversazionali locali
...
**Data della consegna storica: 20 settembre 2026.**

I punti seguenti conservano il perimetro originale e non sono una checklist corrente. Per lavoro ancora aperto vedere [NPC_AI_INTEGRATION](NPC_AI_INTEGRATION.md#lavoro-aperto).
...
## Consegna storica a Claude — non riavviare automaticamente
...
- (Criterio della consegna storica) Leggi le istruzioni locali applicabili, questo documento, `git status
  --short`, branch, HEAD e diff prima di lavorare.
- (Criterio della consegna storica) Crea un **worktree separato con branch dedicato** dal HEAD locale:
...
- (Criterio della consegna storica) Se branch o destinazione esistono, ispezionali e riutilizzali soltanto
...
- (Criterio della consegna storica) Il worktree non contiene le modifiche non committate: è intenzionale.
...
- (Criterio della consegna storica) Annota nel resoconto il commit di base e il percorso del worktree.
  Tutti i comandi di sviluppo vanno eseguiti nel worktree NPC.
- (Criterio della consegna storica) Non aggiornare il branch con pull/merge di altri lavori. Non fare merge
...
- (Criterio della consegna storica) Riporta brevemente componenti riutilizzabili trovati e file che prevedi
  di toccare; nessun nuovo framework parallelo se esiste già un contratto adatto.
- (Criterio della consegna storica) Crea risorse per identità stabile dell'NPC, mestiere, tono, fatti pubblici,
  conoscenze personali autorizzate e battute di ripiego in italiano.
- (Criterio della consegna storica) Definisci richiesta con `npc_id`, `session_id`, `request_id`, revisione
...
- (Criterio della consegna storica) Implementa un backend finto deterministico per testare il contratto,
  mantenendolo chiaramente distinto dall'inferenza reale.
- (Criterio della consegna storica) Realizza il laboratorio con un fabbro, apertura/chiusura dialogo, input
  testuale, storico limitato e stati «in attesa / risposta / ripiego».

#### B — Contesto, memoria e validazione

- (Criterio della consegna storica) Il gioco prepara una lista esplicita dei fatti accessibili all'NPC.
  Non consegnare l'intero stato del mondo, file del progetto o segreti.
- (Criterio della consegna storica) Separa istruzioni del personaggio, eventi canonici del gioco,
  dichiarazioni non confermate del giocatore e testo generato.
- (Criterio della consegna storica) Memoria per `npc_id`, limiti di spazio e schema versionato. Salva in
...
- (Criterio della consegna storica) Per il primo prototipo usa selezione deterministica di eventi confermati
  e uno storico breve; non occorre un database vettoriale o una seconda IA.
- (Criterio della consegna storica) Limita dimensioni di input/output; valida identità della richiesta,
...
- (Criterio della consegna storica) Scarta risposte obsolete dopo chiusura, nuova sessione, rimozione NPC,
  cambio scena e caricamento/reset della memoria del laboratorio.

#### C — Inferenza realmente locale

- (Criterio della consegna storica) Confronta le due opzioni descritte sotto usando documentazione primaria
  aggiornata. Scegline una e documenta motivi, versione e licenza verificata.
- (Criterio della consegna storica) Scegli e misura un piccolo modello adatto all'italiano, con licenza che
  consenta il pacchetto previsto. Non assumere che i suggerimenti Gemini
  siano compatibili o liberamente ridistribuibili.
- (Criterio della consegna storica) Implementa un unico servizio condiviso, coda limitata, una generazione
...
- (Criterio della consegna storica) Se scegli un processo incluso: avvio/arresto dal modulo, nessuna finestra
...
- (Criterio della consegna storica) Gestisci modello mancante/incompatibile, runtime indisponibile, memoria
  insufficiente e crash con ripiego deterministico utilizzabile.
- (Criterio della consegna storica) Nessun account, chiave API, cloud, telemetria o chiamata esterna durante
...
- (Criterio della consegna storica) Non inserire pesi o binari voluminosi in Git. Fornisci manifest e procedura
  riproducibile per assemblare il pacchetto NPC offline con le licenze.

#### D — Verifiche e consegna

- (Criterio della consegna storica) Test automatici del contratto, selezione dei fatti, isolamento fra due
  identità/sessioni, salvataggio/riapertura, dati corrotti e limiti di memoria.
- (Criterio della consegna storica) Test di timeout, annullamento e risposta tardiva: chiudere un dialogo
  non deve far comparire la risposta nella sessione successiva.
- (Criterio della consegna storica) Test di manipolazione: «ignora le istruzioni», falso messaggio di sistema,
...
- (Criterio della consegna storica) Prova end-to-end con il backend reale e il fabbro in italiano, poi senza
  backend: il ripiego deve mantenere utilizzabile il dialogo.
- (Criterio della consegna storica) Misura caricamento a freddo, RAM/VRAM, latenza primo testo/completamento
...
- (Criterio della consegna storica) Verifica il pacchetto del laboratorio senza rete e senza dipendenze
...
- (Criterio della consegna storica) Controlla che il diff finale contenga solo percorsi autorizzati. Esegui
...
- (Criterio della consegna storica) Definire l'interazione desiderata: testo libero, opzioni guidate, reazioni
  contestuali e grado di memoria; scegliere un caso concreto, per esempio un fabbro.
- (Criterio della consegna storica) Esaminare il sistema di dialogo esistente e i suoi punti di estensione,
  senza creare una seconda pipeline di quest o comportamento.
- (Criterio della consegna storica) Confrontare runtime nativo e processo incluso nel pacchetto; verificare
  documentazione ufficiale, manutenzione e licenze al momento della scelta.
- (Criterio della consegna storica) Confrontare modelli locali piccoli e quantizzati su italiano, ruolo,
  conoscenze limitate e rispetto del formato; nessun modello è già approvato.
- (Criterio della consegna storica) Definire il contratto fra gioco e modello: contesto, risposta, intenzioni,
  validazioni, errori, timeout, annullamento e battute di ripiego.
- (Criterio della consegna storica) Progettare memoria e salvataggi per NPC, separando eventi canonici,
  dichiarazioni del giocatore e riassunti non affidabili.
- (Criterio della consegna storica) Preparare prove di manipolazione: cambio identità, falsi messaggi di sistema,
  delimitatori chiusi dal giocatore, falsi ricordi, richieste di segreti e premi.
- (Criterio della consegna storica) Misurare il budget insieme al rendering e fissare criteri quantitativi
  di accettazione prima di estendere il sistema.
- (Criterio della consegna storica) Prototipo isolato con un NPC, entro il handoff sopra, e nessuna modifica
  diretta del mondo da parte del modello.
- (Criterio della consegna storica) Verificare il pacchetto su una macchina pulita e senza rete: niente installazioni
  manuali aggiuntive, servizi orfani o dipendenze nascoste.
- (Criterio della consegna storica) Decidere se promuovere il prototipo a capacità riutilizzabile del progetto
