...
func run() -> void:
	...
	var ivy: Node3D=house.get_node("RecipeDetails/EderaFacciata")
	check(not ivy.collision_enabled,"ivy does not obstruct movement")
	var leaves: PackedVector3Array=ivy.get_node("_GeneratedRecipeDetail/DetailMesh").mesh.get_faces()
	ivy.rebuild()
	check(leaves==ivy.get_node("_GeneratedRecipeDetail/DetailMesh").mesh.get_faces(),"ivy scatter is deterministic")
	ivy.surface_exclusions.append(Rect2(-10,-10,20,20))
	ivy.rebuild()
	check(ivy.triangle_count==0,"manual surface exclusion removes stems and leaves")
	ivy.surface_exclusions.clear(); ivy.rebuild()
	var path_style=load("res://assets/art/chapel_path_surface.tres")
	var route := PackedVector3Array([Vector3.ZERO,Vector3(0,0,8)])
	check(path_style.road_amount(Vector2(0,4),route)>.99,"road keeps a continuous worn centre")
	check(path_style.road_amount(Vector2(4,4),route)<.01,"road field stays local")
	check(path_style.warp(Vector2(12,4),route)==Vector2.ZERO,"edge warping leaves distant terrain unchanged")
