...
func run() -> void:
 for sides in [8,12,16]:
  ...
  tower.face_count=sides; tower.width=8; tower.depth=7; tower.wall_height=5; tower.roof_height=3; tower.tower_roof=1
  ...
  var copy=packed.instantiate(); assert(copy.tower_roof==1 and copy.face_count==sides); copy.free()
  ...
  tower.tower_roof=1; assert(tower.tower_roof==0,"Do not replace a roof used by a staircase")
  tower.free()
 print("CONICAL_TOWERS_GEOMETRY_COLLISION_SAVE_ACCESS_OK")
 quit()
