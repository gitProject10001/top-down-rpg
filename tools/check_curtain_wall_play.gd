...
func run() -> void:
	var example=load("res://scenes/dev/curtain_wall_example.tscn").instantiate()
	var house=example.get_node("Cortina"); example.remove_child(house); own(house,house)
	...
	var gate=play.house._generated.get_node("Door_0")
	await play._walk(Vector3(0,0,0),160)
	assert(play.player.position.z>0.9,"Closed portone blocks passage")
	assert(gate.toggle(play.player.global_position))
	for i in 45: await physics_frame
	await play._walk(Vector3(0,0,-3),240)
	assert(play.player.position.z<-2 and not play.inside,"Open portone allows through-passage outdoors")
	assert(play.house.gate_open,"Gate state stored on authored node")
	await play._walk(Vector3(0,0,5),300)
	...
	if DisplayServer.get_name()!="headless":
		...
		root.get_texture().get_image().save_png("res://captures/balcony_attachment/curtain_wall_play.png")
	...
	print("CURTAIN_GATE_PASSAGE_WALKWAY_ROUNDTRIP_OK")
	play.queue_free()
