...
func run() -> void:
	var example=load("res://scenes/dev/castle_raised_courtyard_example.tscn").instantiate()
	...
	assert(play.player.position.y>1.9,"Player climbed courtyard access")
	...
	if DisplayServer.get_name()!="headless":
		...
		root.get_texture().get_image().save_png("res://captures/balcony_attachment/raised_courtyard_play.png")
	await play._walk(keep.to_global(Vector3(0,0,0)),180)
	await play._walk(keep.to_global(Vector3(0,0,1.1)),100)
	print("RAISED_COURTYARD_ACCESSORY_ENTER_RETURN_OK")
	...
	await play._walk(Vector3(8,0,4),350)
	assert(play.player.position.y<1.1 and not play.inside,"Returns to original ground through gate")
	print("RAISED_COURTYARD_KEEP_THREE_FLOORS_EXIT_OK")
