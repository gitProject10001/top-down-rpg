...
func run() -> void:
	var sides := int(OS.get_cmdline_user_args()[0]) if not OS.get_cmdline_user_args().is_empty() else 12
	var example=load("res://scenes/dev/tower_roof_stair_example.tscn").instantiate()
	var house=example.get_node("TorreOttagonale"); example.remove_child(house); house.face_count=sides; own(house,house)
	var packed := PackedScene.new(); assert(packed.pack(house)==OK); assert(ResourceSaver.save(packed,"res://scenes/dev/tower_%d_interior_example.tscn"%sides)==OK); house.free(); example.free()
	...
	if DisplayServer.get_name()!="headless":
		...
		root.get_texture().get_image().save_png("res://captures/balcony_attachment/segmented_interior_ground.png")
	...
	if DisplayServer.get_name()!="headless":
		await RenderingServer.frame_post_draw
		root.get_texture().get_image().save_png("res://captures/balcony_attachment/segmented_interior_upper.png")
	...
	if DisplayServer.get_name()!="headless":
		await RenderingServer.frame_post_draw
		root.get_texture().get_image().save_png("res://captures/balcony_attachment/segmented_interior_roof.png")
	...
	print("SEGMENTED_TOWER_ROOF_ROUNDTRIP_OK sides=",sides)
