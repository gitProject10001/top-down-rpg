...
func run() -> void:
	var example=load("res://scenes/dev/square_tower_example.tscn").instantiate()
	var house=example.get_node("TorreQuadrata"); example.remove_child(house); own(house,house)
	...
	if DisplayServer.get_name()!="headless":
		...
		root.get_texture().get_image().save_png("res://captures/balcony_attachment/square_tower_play.png")
	...
	print("SQUARE_TOWER_REAL_PLAYER_ASCEND_OUTDOORS_DESCEND_OK")
	play.queue_free()
