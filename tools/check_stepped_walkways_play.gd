...
func run() -> void:
	var example=load("res://scenes/dev/castle_stepped_walkways_example.tscn").instantiate()
	...
	for x in [0.0,16.0]:
		...
		for i in 30: await physics_frame
		for point in [Vector3(x,7.78,-8),Vector3(x,9.78,-16),Vector3(x,7.78,-8),Vector3(x,5.78,0)]:
			...
			assert(absf(feet-(5.78+4.0*clampf((-play.player.position.z-4.5)/7.0,0,1)))<0.15 and not play.inside,"Continuous collision at both elevations")
			if point==Vector3(16,7.78,-8) and DisplayServer.get_name()!="headless":
				await RenderingServer.frame_post_draw
				root.get_texture().get_image().save_png("res://captures/balcony_attachment/stepped_walkway_play.png")
	print("STEPPED_WALKWAYS_BOTH_SIDES_ASCEND_DESCEND_OK")
