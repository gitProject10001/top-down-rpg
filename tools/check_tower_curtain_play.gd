...
func run() -> void:
	var example=load("res://scenes/dev/tower_curtain_example.tscn").instantiate()
	...
	await play._walk(Vector3(10,5.78,0),350)
	feet=play.player.position.y+shape.position.y-shape.shape.height*0.5
	assert(play.player.position.x>9.5 and feet>5.65 and not play.inside,"Player crosses tower parapet and curtain end without falling")
	if DisplayServer.get_name()!="headless":
		await RenderingServer.frame_post_draw
		root.get_texture().get_image().save_png("res://captures/balcony_attachment/tower_curtain_play.png")
	...
	print("TOWER_CURTAIN_REAL_PLAYER_ROUNDTRIP_OK")
