...
func run(plugin: EditorPlugin) -> void:
	...
	var organic_scene=load("res://scenes/dev/village_organic_example.tscn").instantiate(); scene.add_child(organic_scene)
	var organic=organic_scene.get_node("Villaggio")
	EditorInterface.get_selection().clear(); EditorInterface.get_selection().add_node(organic.guides(4)[1]); plugin._selection()
	var route_count: int=organic.guides(5).size(); plugin._edit_court_route()
	assert(organic.guides(5).size()==route_count+1 and plugin.tabs.current_tab==1)
	var editable=plugin.gizmos.focus; assert(editable.kind==5)
	var old_widths: PackedFloat32Array=editable.point_widths.duplicate()
	plugin.width_vertex.select(0); plugin._set_vertex_width(1.2)
	assert(is_equal_approx(editable.point_widths[0],1.2),"Vertex width did not change"); history.undo(); assert(editable.point_widths==old_widths,"Width undo failed"); history.redo()
	plugin._preview_routes(); assert(is_instance_valid(plugin.route_preview) and not plugin.route_proposal.is_empty(),plugin.status.text)
	var old_material=organic.surface_material
	plugin._apply_routes(); assert(organic.surface_material!=old_material); history.undo(); assert(organic.surface_material==old_material); history.redo()
	plugin._clear_route_preview()
	print("VILLAGE_EDITOR_MANUAL_ROUTE_WIDTH_PREVIEW_APPLY_UNDO_OK")
