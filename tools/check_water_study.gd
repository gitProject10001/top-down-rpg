...
func check() -> void:
    ...
    var toggle:=InputEventKey.new();toggle.pressed=true;toggle.keycode=KEY_F7
    scene._unhandled_key_input(toggle)
    assert(water.simulation_enabled)
    assert(water._surface.material_override.get_shader_parameter("simulation_enabled"))
    var impulse:=InputEventKey.new();impulse.pressed=true;impulse.keycode=KEY_O
    scene._unhandled_key_input(impulse)
    assert(water.wave_field.max_height()>0,"Interaction injects simulated waves")
    var before: PackedFloat32Array=water.wave_field.height.duplicate()
    water.ripple(Vector3(-20,0,20))
    assert(water.wave_field.height==before,"Dry impulses leave simulation unchanged")
    scene._unhandled_key_input(toggle)
    assert(not water.simulation_enabled)
    assert(not water._surface.material_override.get_shader_parameter("simulation_enabled"))
