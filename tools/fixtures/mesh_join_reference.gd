...
static func append(target: ArrayMesh, source: Mesh, transform: Transform3D, cutters: Array, scheduler: Node=null) -> void:
	if cutters.is_empty() and transform==Transform3D.IDENTITY:
		# Concatenation needs no triangle expansion, tangent generation or indexing.
		for surface in source.get_surface_count():
			target.add_surface_from_arrays(Mesh.PRIMITIVE_TRIANGLES,source.surface_get_arrays(surface))
	...
	for surface in source.get_surface_count():
		...
		for triangle in range(0,count,3):
			if scheduler!=null and triangle%96==0:
				if not await scheduler.yield_build(): return
			var polygon: Array=[]
			for corner in 3:
