...
func run() -> void:
	...
	for index in 3:
		...
		house.width=8; house.depth=8; house.wall_height=6; house.roof_height=3.5; house.tower_roof=1; house.wall_finish=1
	...
	assert(ResourceSaver.save(scene,"res://scenes/dev/conical_towers_example.tscn")==OK)
	...
	var label := Label.new(); label.text="A07.5 · COPERTURE CONICHE\n8 / 12 / 16 facce · altezza indipendente · tegole in rilievo"; label.position=Vector2(30,30); label.add_theme_font_size_override("font_size",20); ui.add_child(label)
	await process_frame; await RenderingServer.frame_post_draw
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://captures/balcony_attachment"))
	root.get_texture().get_image().save_png("res://captures/balcony_attachment/conical_towers.png")
	quit()
