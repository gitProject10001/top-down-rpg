...
func run() -> void:
	...
	var house=preload("res://addons/house_builder/curtain_wall.gd").new(); house.name="Cortina"
	world.add_child(house); house.owner=world
	var stairs=preload("res://addons/house_builder/exterior_stair.gd").new(); stairs.name="ScalaCamminamento"; stairs.side=1; house.add_child(stairs); stairs.owner=world
	house.rebuild()
	...
	assert(ResourceSaver.save(scene,"res://scenes/dev/curtain_wall_example.tscn")==OK)
	print("MULTI_VOLUME_EXAMPLE_SAVED")
	if DisplayServer.get_name()=="headless": quit(); return
	camera.size=24; camera.look_at(Vector3(0,2.5,0))
	root.msaa_3d=Viewport.MSAA_4X
	for frame in 30: await process_frame
	for layer in root.find_children("*","CanvasLayer",true,false): layer.hide()
	var ui := CanvasLayer.new(); root.add_child(ui)
	var label := Label.new(); label.text="CORTINA CON PORTONE\nMuro parametrico · passaggio reale · camminamento"; label.position=Vector2(30,30); label.add_theme_font_size_override("font_size",20); ui.add_child(label)
	await process_frame; await RenderingServer.frame_post_draw
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://captures/balcony_attachment"))
	root.get_texture().get_image().save_png("res://captures/balcony_attachment/curtain_wall.png")
	quit()
