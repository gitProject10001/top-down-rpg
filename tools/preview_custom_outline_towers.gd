...
func run() -> void:
	...
	for index in 3:
		...
		house.face_count=8;
		var outline=house.regular_outline()
		for i in outline.size(): outline[i].x+=outline[i].y*[0.0,0.45,-0.45][index]
		house.custom_outline=outline; house.name="Torre_%d"%house.face_count
	...
	assert(ResourceSaver.save(scene,"res://scenes/dev/custom_outline_towers_example.tscn")==OK)
	...
	var label := Label.new(); label.text="A07.8 · SAGOME CONVESSE\nRegolare / deformata destra / deformata sinistra"; label.position=Vector2(30,30); label.add_theme_font_size_override("font_size",20); ui.add_child(label)
	await process_frame; await RenderingServer.frame_post_draw
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://captures/balcony_attachment"))
	root.get_texture().get_image().save_png("res://captures/balcony_attachment/custom_outline_towers.png")
	quit()
