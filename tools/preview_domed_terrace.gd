...
func run() -> void:
	...
	world.add_child(group); group.owner=world
	for node in group.find_children("*","Node",true,false): node.owner=world
	group.get_node("Terrazza").rebuild(); group.get_node("PadiglioneCupola").rebuild()
	var camera := Camera3D.new(); camera.name="Camera"; camera.projection=Camera3D.PROJECTION_ORTHOGONAL; camera.size=28
	...
	assert(ResourceSaver.save(scene,"res://scenes/dev/domed_terrace_example.tscn")==OK)
	print("MULTI_VOLUME_EXAMPLE_SAVED")
	if DisplayServer.get_name()=="headless": quit(); return
	camera.size=28; camera.look_at(Vector3(0,2.5,0))
	root.msaa_3d=Viewport.MSAA_4X
	for frame in 30: await process_frame
	for layer in root.find_children("*","CanvasLayer",true,false): layer.hide()
	var ui := CanvasLayer.new(); root.add_child(ui)
	var label := Label.new(); label.text="A07.6b · EDIFICIO TERRAZZATO\nScala esterna · terrazza · padiglione a cupola"; label.position=Vector2(30,30); label.add_theme_font_size_override("font_size",20); ui.add_child(label)
	await process_frame; await RenderingServer.frame_post_draw
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://captures/balcony_attachment"))
	root.get_texture().get_image().save_png("res://captures/balcony_attachment/domed_terrace.png")
	quit()
