...
func run() -> void:
	...
	group.rebuild()
	group.apply_layout(group.resize_proposal(Vector2(20,18)))
	var camera := Camera3D.new(); camera.name="Camera"; camera.projection=Camera3D.PROJECTION_ORTHOGONAL; camera.size=42
	camera.position=Vector3(33,32,17); world.add_child(camera); camera.owner=world; camera.look_at(Vector3(10,2.8,-9))
	var scene := PackedScene.new(); assert(scene.pack(world)==OK)
	assert(ResourceSaver.save(scene,"res://scenes/dev/castle_enclosure_resized_example.tscn")==OK)
	print("MULTI_VOLUME_EXAMPLE_SAVED")
	if DisplayServer.get_name()=="headless": quit(); return
	camera.size=42; camera.look_at(Vector3(10,2.5,-9))
	root.msaa_3d=Viewport.MSAA_4X
	for frame in 30: await process_frame
	for layer in root.find_children("*","CanvasLayer",true,false): layer.hide()
	var ui := CanvasLayer.new(); root.add_child(ui)
	var label := Label.new(); label.text="PRIMO RECINTO FORTIFICATO\nRecinto ridimensionato 20 × 18 m tra le torri · dettagli conservati"; label.position=Vector2(30,30); label.add_theme_font_size_override("font_size",20); ui.add_child(label)
	await process_frame; await RenderingServer.frame_post_draw
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://captures/balcony_attachment"))
	root.get_texture().get_image().save_png("res://captures/balcony_attachment/castle_enclosure_resized.png")
	quit()
