...
func run() -> void:
	...
	assert(ResourceSaver.save(scene,"res://scenes/dev/flat_roof_example.tscn")==OK)
	...
	var label := Label.new(); label.text="COPERTURE PIANE\nParapetto opzionale · volumi e aperture indipendenti"; label.position=Vector2(30,30); label.add_theme_font_size_override("font_size",20); ui.add_child(label)
	await process_frame; await RenderingServer.frame_post_draw
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://captures/balcony_attachment"))
	root.get_texture().get_image().save_png("res://captures/balcony_attachment/flat_roof.png")
	quit()
