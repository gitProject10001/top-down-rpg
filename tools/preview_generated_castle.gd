...
func run() -> void:
	...
	var request=preload("res://addons/castle_generator/request.gd").new(); request.seed_value=17
	var result=preload("res://addons/castle_generator/planner.gd").generate(request)
	assert(result.errors.is_empty())
	var group=preload("res://addons/castle_generator/materializer.gd").create(result.plan)
	world.add_child(group)
	own_authored(group,world)
	group.rebuild()
	var camera := Camera3D.new(); camera.name="Camera"; camera.projection=Camera3D.PROJECTION_ORTHOGONAL; camera.size=48
	camera.position=Vector3(43,38,17); world.add_child(camera); camera.owner=world; camera.look_at(Vector3(13,2.8,-13))
	var scene := PackedScene.new(); assert(scene.pack(world)==OK)
	assert(ResourceSaver.save(scene,"res://scenes/dev/castle_generated_seed17.tscn")==OK)
	print("MULTI_VOLUME_EXAMPLE_SAVED")
	if DisplayServer.get_name()=="headless": quit(); return
	camera.size=48; camera.look_at(Vector3(13,2.5,-13))
	root.msaa_3d=Viewport.MSAA_4X
	for frame in 30: await process_frame
	for layer in root.find_children("*","CanvasLayer",true,false): layer.hide()
	var ui := CanvasLayer.new(); root.add_child(ui)
	var label := Label.new(); label.text="COMPOSITORE · SEED 17\nPiano generato → builder → nodi modificabili"; label.position=Vector2(30,30); label.add_theme_font_size_override("font_size",20); ui.add_child(label)
	await process_frame; await RenderingServer.frame_post_draw
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://captures/balcony_attachment"))
	root.get_texture().get_image().save_png("res://captures/balcony_attachment/castle_generated_seed17.png")
	quit()

func own_authored(node: Node, scene: Node) -> void:
	if str(node.name).begins_with("_"): return
	node.owner=scene
	for child in node.get_children(): own_authored(child,scene)
