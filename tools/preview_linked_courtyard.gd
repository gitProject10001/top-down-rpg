...
func run() -> void:
	...
	assert(not proposal.has("error"))
	group.add_child(proposal.court); proposal.court.owner=world
	for path in proposal.positions: group.get_node(path).position=proposal.positions[path]
	for wall in group.curtains():
		if not wall.gate_enabled: wall.allow_sloped_walkway=true
	group.rebuild()
	var height_edit: Dictionary=proposal.court.height_proposal(1.6); assert(not height_edit.has("error")); proposal.court.apply_height_state(height_edit)
	...
	assert(ResourceSaver.save(scene,"res://scenes/dev/castle_linked_courtyard_example.tscn")==OK)
	...
	var label := Label.new(); label.text="CORTE RIALZATA\nCorte e fondazioni +1.6 m · accesso dalla corte bassa"; label.position=Vector2(30,30); label.add_theme_font_size_override("font_size",20); ui.add_child(label)
	await process_frame; await RenderingServer.frame_post_draw
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://captures/balcony_attachment"))
	root.get_texture().get_image().save_png("res://captures/balcony_attachment/castle_linked_courtyard.png")
	quit()
