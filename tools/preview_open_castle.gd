...
func run() -> void:
	...
	assert(ResourceSaver.save(scene,"res://scenes/dev/castle_open_courtyard_example.tscn")==OK)
	...
	var label := Label.new(); label.text="CORTE APERTA\nEdifici sul fondo · spazio libero · visibilità nel Play"; label.position=Vector2(30,30); label.add_theme_font_size_override("font_size",20); ui.add_child(label)
	await process_frame; await RenderingServer.frame_post_draw
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://captures/balcony_attachment"))
	root.get_texture().get_image().save_png("res://captures/balcony_attachment/castle_open_courtyard.png")
	quit()
