...
func run() -> void:
	...
	var samples=preload("res://addons/rock_builder/formation.gd").new(); samples.name="PareteRocciosa"
	...
	assert(ResourceSaver.save(scene,"res://scenes/dev/rock_formation_example.tscn")==OK)
