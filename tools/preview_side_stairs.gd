...
func run() -> void:
	...
	var house := House.new(); house.name="CasaConTerrazza"; house.width=6.0; house.depth=7.0; house.wall_height=5.4; house.roof_height=2.5
	...
	var balcony=load("res://addons/house_builder/balcony.gd").new(); balcony.name="TerrazzaIngresso"; balcony.support_posts=true; balcony.exterior_stairs=true; balcony.projection=2.4; balcony.elevation=2.8; balcony.balcony_width=4.0; balcony.door_open=true
	...
	balcony.exterior_stairs=false
	var exterior=preload("res://addons/house_builder/exterior_stair.gd").new(); exterior.name="ScalaDestra"; exterior.side=1
	balcony.add_child(exterior); exterior.owner=world; house.rebuild()
	...
	assert(ResourceSaver.save(scene,"res://scenes/dev/side_stairs_example.tscn")==OK)
	print("SIDE_STAIRS_EXAMPLE_SAVED")
	if DisplayServer.get_name()=="headless": quit(); return
	camera.size=19; camera.look_at(Vector3(0,2.2,2.0))
	root.msaa_3d=Viewport.MSAA_4X
	for frame in 30: await process_frame
	for layer in root.find_children("*","CanvasLayer",true,false): layer.hide()
	var ui := CanvasLayer.new(); root.add_child(ui)
	var label := Label.new(); label.text="SCALA INDIPENDENTE · BORDO DESTRO\nIl parapetto apre il varco sul lato scelto"; label.position=Vector2(30,30); label.add_theme_font_size_override("font_size",20); ui.add_child(label)
	await process_frame; await RenderingServer.frame_post_draw
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://captures/balcony_attachment"))
	root.get_texture().get_image().save_png("res://captures/balcony_attachment/side_stairs.png")
	quit()
