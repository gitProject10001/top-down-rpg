...
func run() -> void:
	...
	for name in ["TorreNordEst","TorreNordOvest"]:
		...
		plan.floor_height=3.4; plan.get_node("PianoTerra/ScalaPrimoPiano").dimensions.y=3.4; plan.rebuild()
	...
	assert(ResourceSaver.save(scene,"res://scenes/dev/castle_sloped_walkways_example.tscn")==OK)
	...
	var label := Label.new(); label.text="QUOTE E CAMMINAMENTI\nTorri posteriori +1.2 m · raccordi percorribili"; label.position=Vector2(30,30); label.add_theme_font_size_override("font_size",20); ui.add_child(label)
	await process_frame; await RenderingServer.frame_post_draw
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://captures/balcony_attachment"))
	root.get_texture().get_image().save_png("res://captures/balcony_attachment/castle_sloped_walkways.png")
	quit()
