...
func run() -> void:
	...
	var house=preload("res://addons/house_builder/tower_factory.gd").create()
	world.add_child(house); house.owner=world
	var stairs=preload("res://addons/house_builder/exterior_stair.gd").new(); stairs.name="ScalaTetto"; stairs.side=1; house.add_child(stairs); stairs.owner=world
	house.rebuild()
	...
	assert(ResourceSaver.save(scene,"res://scenes/dev/square_tower_example.tscn")==OK)
	print("MULTI_VOLUME_EXAMPLE_SAVED")
	if DisplayServer.get_name()=="headless": quit(); return
	camera.size=20; camera.look_at(Vector3(0,2.5,0))
	root.msaa_3d=Viewport.MSAA_4X
	for frame in 30: await process_frame
	for layer in root.find_children("*","CanvasLayer",true,false): layer.hide()
	var ui := CanvasLayer.new(); root.add_child(ui)
	var label := Label.new(); label.text="TORRE QUADRATA\nMerli parametrici · accesso al tetto · builder esistente"; label.position=Vector2(30,30); label.add_theme_font_size_override("font_size",20); ui.add_child(label)
	await process_frame; await RenderingServer.frame_post_draw
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://captures/balcony_attachment"))
	root.get_texture().get_image().save_png("res://captures/balcony_attachment/square_tower.png")
	quit()
