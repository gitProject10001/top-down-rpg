...
func run() -> void:
	...
	assert(ResourceSaver.save(scene,"res://scenes/dev/terrace_stairs_example.tscn")==OK)
	print("TERRACE_EXAMPLE_SAVED")
	...
	var label := Label.new(); label.text="TERRAZZA SU PILASTRI + SCALA ESTERNA\nPiano, accesso e lunghezza della scala collegati"; label.position=Vector2(30,30); label.add_theme_font_size_override("font_size",20); ui.add_child(label)
	await process_frame; await RenderingServer.frame_post_draw
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://captures/balcony_attachment"))
	root.get_texture().get_image().save_png("res://captures/balcony_attachment/terrace_stairs.png")
	quit()
