...
func run() -> void:
	...
	house.name="TorreOttagonale"; house.width=8.0; house.depth=8.0; house.wall_height=5.6; house.roof_height=1.0
	...
	plan.rebuild()
	var curtain=preload("res://addons/house_builder/curtain_wall.gd").new(); curtain.name="Cortina"; curtain.width=8.0; curtain.connect_to_tower=true; house.add_child(curtain); curtain.owner=world; curtain.rebuild()
	house.rebuild()
	...
	assert(ResourceSaver.save(scene,"res://scenes/dev/tower_curtain_example.tscn")==OK)
	print("MULTI_VOLUME_EXAMPLE_SAVED")
	if DisplayServer.get_name()=="headless": quit(); return
	plan.preview_inside=false; plan.active_floor=1; plan.editor_view()
	camera.size=23; camera.look_at(Vector3(3,2.5,0))
	root.msaa_3d=Viewport.MSAA_4X
	for frame in 30: await process_frame
	plan.editor_view()
	for layer in root.find_children("*","CanvasLayer",true,false): layer.hide()
	var ui := CanvasLayer.new(); root.add_child(ui)
	var label := Label.new(); label.text="TORRE E CORTINA\nRaccordo parametrico · camminamento continuo · parapetti aperti"; label.position=Vector2(30,30); label.add_theme_font_size_override("font_size",20); ui.add_child(label)
	await process_frame; await RenderingServer.frame_post_draw
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://captures/balcony_attachment"))
	root.get_texture().get_image().save_png("res://captures/balcony_attachment/tower_curtain.png")
	quit()
