...
func run() -> void:
	...
	house.name="TorreOttagonale"; house.width=7.2; house.depth=7.2; house.wall_height=5.6; house.roof_height=1.0
	...
	assert(ResourceSaver.save(scene,"res://scenes/dev/tower_roof_stair_example.tscn")==OK)
	print("MULTI_VOLUME_EXAMPLE_SAVED")
	if DisplayServer.get_name()=="headless": quit(); return
	plan.preview_inside=true; plan.active_floor=1; plan.editor_view()
	...
	var label := Label.new(); label.text="TORRE OTTAGONALE\nPrimo piano → tetto · scala modificabile · apertura automatica"; label.position=Vector2(30,30); label.add_theme_font_size_override("font_size",20); ui.add_child(label)
	await process_frame; await RenderingServer.frame_post_draw
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://captures/balcony_attachment"))
	root.get_texture().get_image().save_png("res://captures/balcony_attachment/tower_roof_stair.png")
	quit()
