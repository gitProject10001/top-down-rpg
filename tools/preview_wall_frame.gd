...
func run() -> void:
	...
	for pair in [[0,1],[0,-1],[1,-1]]:
		...
		link.support_a=supports.get_child(pair[0]).support_id; link.support_b=supports.get_child(pair[1]).support_id if pair[1]>=0 else ""; link.endpoint_mode=1 if pair[1]<0 else 0
	...
	assert(ResourceSaver.save(scene,"res://scenes/dev/wall_frame_example.tscn")==OK)
	...
	var label := Label.new(); label.text="PORTICO + TETTOIA\nDue pali esterni · travi agganciate alla parete"; label.position=Vector2(30,30); label.add_theme_font_size_override("font_size",20); ui.add_child(label)
	await process_frame; await RenderingServer.frame_post_draw
	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("res://captures/balcony_attachment"))
	root.get_texture().get_image().save_png("res://captures/balcony_attachment/wall_frame.png")
	camera.size=10; camera.position=Vector3(8,6,16); camera.look_at(Vector3(0,2,5))
	label.text="DETTAGLIO TELAIO\nDue pali esterni e agganci alla parete"
	for frame in 4: await process_frame
	await RenderingServer.frame_post_draw
	root.get_texture().get_image().save_png("res://captures/balcony_attachment/wall_frame_detail.png")
	quit()
